  ...
  angular.module('mgcrea.ngStrap.tooltip', [ 'mgcrea.ngStrap.core', 'mgcrea.ngStrap.helpers.dimensions' ]).provider('$tooltip', function() {
    ...
    this.$get = [ '$window', '$rootScope', '$bsCompiler', '$q', '$templateCache', '$http', '$animate', '$sce', 'dimensions', '$$rAF', '$timeout', function($window, $rootScope, $bsCompiler, $q, $templateCache, $http, $animate, $sce, dimensions, $$rAF, $timeout) {
      ...
      function TooltipFactory(element, config) {
        ...
        var nodeName = (typeof element === 'object')?element[0].nodeName.toLowerCase():'';
        ...
        function bindTriggerEvents() {
          ...
          angular.forEach(triggers, function(trigger) {
            if (trigger === 'click') {
              ...
              (typeof element === 'object')?element.on(trigger === 'hover' ? 'mouseenter' : 'focus', $tooltip.enter):'';
              (typeof element === 'object')?element.on(trigger === 'hover' ? 'mouseleave' : 'blur', $tooltip.leave):'';
              (typeof element === 'object')?nodeName === 'button' && trigger !== 'hover' && element.on(isTouch ? 'touchstart' : 'mousedown', $tooltip.$onFocusElementMouseDown):'';
            }
          });
        }
